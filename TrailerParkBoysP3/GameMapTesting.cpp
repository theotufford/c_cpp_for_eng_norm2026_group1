...
int main () {
    ...
    mapInteractable julian;
    julian.x = 12;
    julian.y = 5;
    julian.rep = 'J';
    julian.placeInteractable(TrailerPark);

    mapInteractable ricky;
    ricky.x = 16;
    ricky.y = 14;
    ricky.rep = 'R';
    ricky.placeInteractable(TrailerPark);

    mapInteractable bubbles;
    bubbles.x = 26;
    bubbles.y = 16;
    bubbles.rep = 'B';
    bubbles.placeInteractable(TrailerPark);

    mapInteractable ice_cubes;
    ice_cubes.x = 10;
    ice_cubes.y = 15;
    ice_cubes.placeInteractable(TrailerPark);

    mapInteractable chicken;
    chicken.x = 26;
    chicken.y = 6;
    chicken.placeInteractable(TrailerPark);

    mapInteractable cat;
    cat.x = 20;
    cat.y = 11;
    cat.placeInteractable(TrailerPark);
    ...
    for (int r = 0; r < 20; r++) { // rows
        for ( int c = 0; c < 31; c++) { // columns 
            if (TrailerPark.posIdentity[r][c].rep != '@' // you need to add any object you dont want over written to this check
                ...
                && TrailerPark.posIdentity[r][c].rep != 'J'     // Julian
                && TrailerPark.posIdentity[r][c].rep != 'R'     // Rickey
                && TrailerPark.posIdentity[r][c].rep != 'B'     // Bubbles
                && TrailerPark.posIdentity[r][c].rep != 'j'     // Julian's object (may not show objects during game)
                && TrailerPark.posIdentity[r][c].rep != 'r'     // Rickey's object
                && TrailerPark.posIdentity[r][c].rep != 'b') {  // Bubbles' object
